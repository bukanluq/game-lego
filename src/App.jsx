...
export default function LegoMinecraft() {
  ...
  useEffect(() => {
    ...
    const studsMesh = new THREE.InstancedMesh(
      studGeo,
      baseplateMat,
      GRID_SIZE * GRID_SIZE
    );
    ...
    const matCache = {};
    COLORS.forEach((c) => {
      matCache[c.hex] = new THREE.MeshLambertMaterial({ color: c.hex });
    });
    ...
    const onKeyUp = (e) => {
      stateRef.current.input.keys[e.code] = false;
    };
    ...
    const onMouseMove = (e) => {
      ...
      stateRef.current.input.pitch = Math.max(
        -Math.PI / 2,
        Math.min(Math.PI / 2, stateRef.current.input.pitch)
      );
    };
    ...
    const onPointerLockChange = () =>
      setIsPlaying(document.pointerLockElement === document.body);
    ...
    const loop = () => {
      ...
      camera.quaternion.setFromEuler(
        new THREE.Euler(
          stateRef.current.input.pitch,
          stateRef.current.input.yaw,
          0,
          "YXZ"
        )
      );
      ...
      const forward = new THREE.Vector3(0, 0, -1)
        .applyEuler(new THREE.Euler(0, stateRef.current.input.yaw, 0))
        .normalize();
      ...
      let moveX = 0,
        moveZ = 0;
      if (stateRef.current.input.keys["KeyW"]) {
        moveX += forward.x;
        moveZ += forward.z;
      }
      if (stateRef.current.input.keys["KeyS"]) {
        moveX -= forward.x;
        moveZ -= forward.z;
      }
      if (stateRef.current.input.keys["KeyA"]) {
        moveX -= right.x;
        moveZ -= right.z;
      }
      if (stateRef.current.input.keys["KeyD"]) {
        moveX += right.x;
        moveZ += right.z;
      }
      ...
      if (checkCollision(player.pos)) {
        if (player.vel.y < 0) {
          // Landing
          player.pos.y =
            Math.ceil(player.pos.y - player.height) + player.height;
      ...
      if (intersects.length > 0) {
        ...
        const point = hit.point
          .clone()
          .add(placeNormal.clone().multiplyScalar(0.1));
        ...
        while (stateRef.current.input.clicks.length > 0) {
          ...
          if (action === "left" && hit.object !== baseplate) {
            // Break
        ...
        for (let x = 0; x < dims.w; x++) {
          for (let z = 0; z < dims.d; z++) {
            if (stateRef.current.grid.has(`${hX + x},${hY},${hZ + z}`))
              willOverlap = true;
          }
        }
  ...
  useEffect(() => {
    stateRef.current.shape = BASE_SHAPES[activeShapeIdx];
  }, [activeShapeIdx]);
  useEffect(() => {
    stateRef.current.color = COLORS[activeColorIdx].hex;
  }, [activeColorIdx]);
    ...
    >
      ...
      {!isPlaying && (
        ...
        >
          ...
          >
            <p>
              <b>WASD</b> to Move | <b>Space</b> to Jump
            </p>
            <p>
              <b>Right Click</b> to Place | <b>Left Click</b> to Break
            </p>
            <p>
              <b>Scroll</b> to Change Shape | <b>1-7</b> to Change Color |{" "}
              <b>R</b> to Rotate
            </p>
          </div>
        </div>
            ...
            <div key={c.id} style={{ textAlign: "center" }}>
              <div
                style={{
                  ...
                  border: `3px solid ${activeColorIdx === i ? "#fff" : "#222"}`,
                  borderRadius: "4px",
                }}
              />
              <div
        ...
        >
          {BASE_SHAPES.map((shape, i) => (
            ...
            >
              <div
                style={{
                  width: `${shape.w * 10}px`,
                  height: `${shape.d * 10}px`,
                  backgroundColor: "#ccc",
                  border: `2px solid ${
                    activeShapeIdx === i ? "#4FC3F7" : "#444"
                  }`,
                  borderRadius: "2px",
                  transition: "all 0.1s",
                }}
          ...
          <div
            style={{
              marginLeft: "15px",
              padding: "8px",
              background: isRotated ? "#4FC3F7" : "#444",
              color: isRotated ? "black" : "white",
              borderRadius: "4px",
              fontFamily: "sans-serif",
              fontWeight: "bold",
              fontSize: "12px",
